...
class ColorTracker:
    def __init__(self, target_color):
        ...
        self.pid_yaw = pid.PID(5.5, 1.0, 0.6)
        self.pid_pitch = pid.PID(10.5, 1.0, 1.2)
        self.yaw = 500
        self.pitch = 150
    ...
    def proc(self, source_image, result_image, color_ranges):
        ...
        if best_c is not None:
            (center_x, center_y), radius = cv2.minEnclosingCircle(best_c[0]) # 最小外接圆(minimum enclosing circle)
            ...
            cv2.putText(result_image, self.target_color, (int(center_x * 2) - 30, int(center_y * 2) - int(radius * 2) - 10),
...
class TrackAndGrabNode(Node):
    ...
    def __init__(self, name):
        ...
        self.enable_disp = self.get_parameter('enable_disp').value
        ...
        self.pick_finish_pub = self.create_publisher(Bool, '~/pick_finish', 1)
    ...
    def init_process(self):
        ...
        if self.get_parameter('start').value:
            ...
            set_servo_position(self.joints_pub, 1, ((1, 500), (2, 650), (3, 151), (4, 70), (5, 500), (10, 150)))
            time.sleep(1)
    ...
    def start_srv_callback(self, request, response):
        ...
        set_servo_position(self.joints_pub, 1, ((1, 500), (2, 650), (3, 151), (4, 70), (5, 500), (10, 150)))
        # set_servo_position(self.joints_pub, 1, ((1, 500), (2, 750), (3, 10), (4, 255), (5, 500), (10, 150)))
        # set_servo_position(self.joints_pub, 1, ((1, 500), (2, 700), (3, 50), (4, 150), (5, 500), (10, 500)))
        time.sleep(1)
    ...
    def pick(self, position):
        ...
        if res.pulse:
            ...
            time.sleep(1.5)
        set_servo_position(self.joints_pub, 0.5, ((10, 580),))
        time.sleep(1)
        position[2] += 0.1
        ...
        if res.pulse:
            ...
            time.sleep(1)
        set_servo_position(self.joints_pub, 1, ((1, 500), (2, 720), (3, 100), (4, 120), (5, 500), (10, 580)))
        time.sleep(1)
        ...
        # set_servo_position(self.joints_pub, 3, ((1, 500), (2, 650), (3, 150), (4, 70), (5, 500), (10, 150)))
        # time.sleep(3.5)
        ...
        for i in range(5):
            self.pick_finish_pub.publish(finish_msg)
            time.sleep(0.5)
                        ...
                        if abs(self.last_pitch_yaw[0] - p_y[0]) < 3 and abs(self.last_pitch_yaw[1] - p_y[1]) < 3:
                            if time.time() - self.stamp > 6:
                                ...
                                if np.isnan(dist):
                                    txt = "DISTANCE ERROR !!!"
                                    return
                                dist += 0.01 # 物体半径补偿(Object radius compensation)
                                dist += 0.015 # 误差补偿(Error compensation)
                                K = depth_camera_info.k
                                self.get_endpoint()
                                position = depth_pixel_to_camera((center_x, center_y), dist, (K[0], K[4], K[2], K[5]))

                                position[0] += 0.00  # rgb相机和深度相机tf有1cm偏移(There is a 1 cm offset between the RGB camera and the depth camera in the TF.)
                                position[1] -= 0.01
                                position[2] += 0.00
