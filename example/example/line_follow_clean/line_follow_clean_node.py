...
# @data:2024/02/28
# @author:aiden
# Clear obstacle during line following(巡线清障)
import os
import cv2
import time
import math
...
MAX_SCAN_ANGLE = 240  # Laser scanning angle, removing the degree that is always obstructed(激光的扫描角度,去掉总是被遮挡的部分degree)
...
class LineFollowCleanNode(Node):
    ...
    def start_srv_callback(self, request, response):
        ...
        line_roi.roi_up.y_min = 250 + step_pixel
        line_roi.roi_up.y_max = 260 + step_pixel
        line_roi.roi_up.scale = 0.0

        line_roi.roi_center.x_min = 0
        line_roi.roi_center.x_max = 640
        line_roi.roi_center.y_min = 310 + step_pixel
        line_roi.roi_center.y_max = 320 + step_pixel
        line_roi.roi_center.scale = 0.1

        line_roi.roi_down.x_min = 0
        line_roi.roi_down.x_max = 640
        line_roi.roi_down.y_min = 370 + step_pixel
        line_roi.roi_down.y_max = 380 + step_pixel
        ...
        object_roi.y_min = 220 + step_pixel
        object_roi.y_max = 380 + step_pixel
                ...
                if self.center is not None:
                    if self.center.y > 100 and abs(self.center.x - self.line_x) < 100 and not self.debug:
                        ...
                        if abs(twist.linear.x) <= 0.0065 and abs(twist.angular.z) <= 0.05:
                            self.count += 1
                            time.sleep(0.01)
                            self.get_logger().info('count1: %s' % self.count)
                            if self.count > 50:
                                self.count = 0
    ...
    def lidar_callback(self, lidar_data):
        # Data size = scanning angle / angle increment per scan(数据大小 = 扫描角度/每扫描一次增加的角度)
        ...
        # Obtain data based on the set(根据设定取数据)
