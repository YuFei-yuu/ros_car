...
- 决策树：`behavior_node` 用简单状态机执行 `go_named_point`、`return_home`、`patrol`，默认启动后自动在两个点之间巡逻。
...
`behavior.launch.py` 默认会打开 RViz 并按配置自动执行 `patrol`，可观察命名点、当前目标点、全局路径、局部路径和 costmap。手动触发任务：
...
5. 启动 `behavior.launch.py`，验证默认巡逻任务。
