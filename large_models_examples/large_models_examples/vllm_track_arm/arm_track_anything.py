...
class ObjectTracker:
    ...
    def cal_speed(self,center_x,center_y,img_w,img_h):
        self.stop_distance = img_h / 2.0
        ...
        tmp = -(self.linear_base_speed - self.linear_pid.output) * 0.06
    ...
    def track_pixel(self, tracker, image,chassis_move=False):
        ...
        if box is not None:
            ...
            if not chassis_move:
                return int(center_x),int(center_y),p1,p2,image
            else:
                linear_speed_,angular_speed_ = self.cal_speed(center_x,center_y,img_w,img_h)
                # self.log.info(f'{self.linear_speed}, {self.angular_speed}')
                return float(linear_speed_), float(angular_speed_), p1,p2,image
        else:
            return 0,0,None,None,image


    def track(self, tracker,image, depth_image,use_pixel=False):
        image, box = self.get_target(tracker,image)
        if box is not None:
            img_h, img_w = image.shape[:2]
            if not use_pixel:
